...
render::Shader *ResourceLoader::loadShader(const std::string &path)
{
    ...
    header << "#define COLORED_SHADOWS " << (enable_dynamic_shadows && shadow_map_color ? 1 : 0) << "\n";
    header << "#define POISSON_FILTER " << (enable_dynamic_shadows && shadow_poisson_filter ? 1 : 0) << "\n";
    ...
    header << "#define ENABLE_SSAA " << (antialiasing == "ssaa" ? 1 : 0) << "\n";
