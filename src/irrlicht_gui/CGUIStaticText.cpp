...
CGUIStaticText::CGUIStaticText(const wchar_t *text, bool border,
        ...
        OverrideColor(img::color8(img::PF_RGBA8, 255, 255, 255, 101)), BGColor(img::PF_RGBA8, 255, 255, 255, 101)),
...
void CGUIStaticText::draw()
{
	...
	if (Background) {
		if (!OverrideBGColorEnabled) // skin-colors can change
            BGColor = skin->getColor(EGDC_3D_FACE);
		...
		if (font) {
			...
			if (!WordWrap) {
				if (VAlign == EGUIA_LOWERRIGHT) {
					frameRect.ULC.Y = frameRect.LRC.Y -
												  font->getDimension(L"A").Height - kerningHeight;
				}
				if (HAlign == EGUIA_LOWERRIGHT) {
					frameRect.ULC.X = frameRect.LRC.X -
				...
				if (VAlign == EGUIA_CENTER) {
					r.ULC.Y = r.getCenter().Y - (totalHeight / 2);
				} else if (VAlign == EGUIA_LOWERRIGHT) {
					r.ULC.Y = r.LRC.Y - totalHeight;
				}

				for (u32 i = 0; i < BrokenText.size(); ++i) {
					if (HAlign == EGUIA_LOWERRIGHT) {
						r.ULC.X = frameRect.LRC.X -
