...
#define DEFINEKEY3(ch) /* single Irrlicht key (e.g. KEY_KEY_X) */ \
    { "KEY_KEY_" TOSTRING(ch), core::KEY_KEY_ ## ch, (wchar_t) *TOSTRING(ch), TOSTRING(ch) },
#define DEFINEKEY4(ch) /* single Irrlicht function key (e.g. KEY_F3) */ \
    { "KEY_F" TOSTRING(ch), core::KEY_F ## ch, L'\0', "F" TOSTRING(ch) },
#define DEFINEKEY5(ch) /* key without Irrlicht keycode */ \
    { ch, core::KEY_KEY_CODES_COUNT, (wchar_t) *ch, ch },
...
static const table_key &lookup_keykey(core::KEY_CODE key)
{
	for (const auto &table_key : table) {
...
MtKey::MtKey(const core::Event::KeyInputEvent &in, bool prefer_character)
{
	if (prefer_character)
        Key = core::KEY_KEY_CODES_COUNT;
	else
