...
CGUIFileOpenDialog::CGUIFileOpenDialog(const wchar_t *title,
		...
		ISimpleTextureSource *tsrc, bool restoreCWD, io::path::char_type *startDir) :
		...
		FileNameText(0), FileList(0), Dragging(false)
{
	...
	CloseButton = GUIButton::addButton(Environment, core::rect<s32>(posx, 3, posx + buttonw, 3 + buttonw),
			tsrc, (IGUIElement *)Environment, -1, L"", skin ? skin->getDefaultText(EGDT_WINDOW_CLOSE) : L"Close");
	...
	OKButton = GUIButton::addButton(Environment, core::rect<s32>(RelativeRect.getWidth() - 80, 30, RelativeRect.getWidth() - 10, 50),
			tsrc, (IGUIElement *)Environment, -1, skin ? skin->getDefaultText(EGDT_MSG_BOX_OK) : L"OK");
	OKButton->setSubElement(true);
	OKButton->setAlignment(EGUIA_LOWERRIGHT, EGUIA_LOWERRIGHT, EGUIA_UPPERLEFT, EGUIA_UPPERLEFT);
	OKButton->grab();

	CancelButton = GUIButton::addButton(Environment, core::rect<s32>(RelativeRect.getWidth() - 80, 55, RelativeRect.getWidth() - 10, 75),
			tsrc, (IGUIElement *)Environment, -1, skin ? skin->getDefaultText(EGDT_MSG_BOX_CANCEL) : L"Cancel");
	CancelButton->setSubElement(true);
	CancelButton->setAlignment(EGUIA_LOWERRIGHT, EGUIA_LOWERRIGHT, EGUIA_UPPERLEFT, EGUIA_UPPERLEFT);
	CancelButton->grab();

	FileBox = new CGUIListBox(Environment, (IGUIElement *)Environment, -1, core::rect<s32>(10, 55, RelativeRect.getWidth() - 90, 230), tsrc, true);
	FileBox->setSubElement(true);
	FileBox->setAlignment(EGUIA_UPPERLEFT, EGUIA_LOWERRIGHT, EGUIA_UPPERLEFT, EGUIA_LOWERRIGHT);
	FileBox->grab();

	FileNameText = new GUIEditBoxWithScrollBar(0, true, Environment, (IGUIElement *)Environment,
			-1, core::rect<s32>(10, 30, RelativeRect.getWidth() - 90, 50), tsrc);
...
IGUIFileOpenDialog *CGUIFileOpenDialog::addFileOpenDialog(const wchar_t *title, bool modal,
			IGUIEnvironment *environment, IGUIElement *parent, s32 id, ISimpleTextureSource *tsrc,
			bool restoreCWD, io::path::char_type *startDir)
{
	if (modal)
		return nullptr;

	IGUIFileOpenDialog *d = new CGUIFileOpenDialog(title, environment, parent, id,
			tsrc, restoreCWD, startDir);
	d->drop();
