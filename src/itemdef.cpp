...
void ItemDefinition::serialize(std::ostream &os, u16 protocol_version) const
{
	...
	if (protocol_version >= 51) {
		...
		os << serializeString16(inventory_image.name);
		os << serializeString16(wield_image.name);
	}
	...
	if (protocol_version >= 51) {
		...
		os << serializeString16(inventory_overlay.name);
		os << serializeString16(wield_overlay.name);
	}
