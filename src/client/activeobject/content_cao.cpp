...
#include "client/player/camera.h" // CameraModes
#include "collision.h"
	...
	for (int layer = 0; layer < MAX_TILE_LAYERS; layer++) {
		for (PreMeshBuffer &p : collector.prebuffers[layer]) {
			...
			u32 shader_id = shdsrc->getShader({
				"object_shader", {"lighting"}, {"common", "fog"}}, p.layer.material_type);
...
void GenericCAO::addToScene(ITextureSource *tsrc, scene::ISceneManager *smgr)
{
			...
			m_prop.visual != OBJECTVISUAL_ITEM)
	{
		...
		u32 shader_id = shader_source->getShader({
			"object_shader", {"lighting"}, {"common", "fog"}}, material_type);
...
void GenericCAO::updateLight(u32 day_night_ratio)
{
	...
	video::SColor light = encode_light(light_at_pos, m_prop.glow, 0.0f);
