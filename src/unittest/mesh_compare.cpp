...
bool operator==(const scene::Vertex3DExt &v1, const scene::Vertex3DExt &v2)
{
	return ((v1.Pos == v2.Pos) &&
		(v1.Normal == v2.Normal) &&
		(v1.Color == v2.Color) &&
		(v1.TCoords == v2.TCoords) &&
		(v1.Aux == v2.Aux));
}

static std::vector<Triangle> expandMesh(const std::vector<scene::Vertex3DExt> &vertices, const std::vector<u16> &indices)
{
...
static std::vector<Triangle> canonicalizeMesh(const std::vector<scene::Vertex3DExt> &vertices, const std::vector<u16> &indices)
{
...
bool checkMeshEqual(const std::vector<scene::Vertex3DExt> &vertices, const std::vector<u16> &indices, const std::vector<Triangle> &expected)
{
	auto actual = canonicalizeMesh(vertices, indices);
	return actual == expected;
}

bool checkMeshEqual(const std::vector<scene::Vertex3DExt> &vertices, const std::vector<u16> &indices, const std::vector<Quad> &expected)
{
