...
CGUIComboBox::CGUIComboBox(IGUIEnvironment *environment, IGUIElement *parent,
		...
		ListButton(nullptr), SelectedText(nullptr), ListBox(nullptr), LastFocus(nullptr), TSrc(tsrc),
		Selected(-1), HAlign(EGUIA_UPPERLEFT), VAlign(EGUIA_CENTER), MaxSelectionRows(5), HasFocus(false),
		ActiveFont(nullptr)
{
	IGUISkin *skin = Environment->getSkin();

	ListButton = GUIButton::addButton(environment, core::recti(0, 0, 1, 1), tsrc, this, -1, L"");
...
IGUIComboBox *CGUIComboBox::addComboBox(IGUIEnvironment *environment,
		const core::rect<s32> &rectangle, IGUIElement *parent, s32 id, ISimpleTextureSource *tsrc)
{
	IGUIComboBox *t = new CGUIComboBox(environment, parent ? parent : (IGUIElement *)environment,
			id, rectangle, tsrc);
	t->drop();
	return t;
}
...
void CGUIComboBox::openCloseMenu()
{
	if (ListBox) {
		...
		ListBox = new CGUIListBox(Environment, this, -1, r, TSrc, false, true, true);
