...
make_install_copy () {
	...
	cp -a include/GL/*.h "$pkgdir/include/GL/"

	if [ -f "lib/libGLEW.a" ]; then
		cp "lib/libGLEW.a" "$pkgdir/"
		echo "Successfully copied libGLEW.a to $pkgdir"
	else
		echo "Error: lib/libGLEW.a not found!"
		exit 1
	fi
}
...
build () {
	make clean
	mkdir -p lib bin tmp/linux/default/static
	make glew.lib.static \
		...
		LDFLAGS.EXTRA="" \
		LDFLAGS.GL="-lGLESv2" \
		CFLAGS.EXTRA="-DGLEW_NO_GLU $CFLAGS" \
		BINDIR="bin" \
		LIBDIR="lib"
	make_install_copy
}
...
_run_build () {
	...
	pushd src
	srcdir=$PWD
	get_tar_archive glew-$abi "https://github.com/nigels-com/glew/releases/download/glew-${glew_ver}/glew-${glew_ver}.tgz"
	popd

	builddir=$srcdir/glew-$abi
