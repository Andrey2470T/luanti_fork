...
void GUIFormSpecMenu::setInitialFocus()
{
	...
	// 5. first focusable (not statictext, not tabheader)
	for (gui::IGUIElement *it : children) {
        if (it->getType() != EGUIET_STATIC_TEXT &&
            it->getType() != EGUIET_TAB_CONTROL) {
			Environment->setFocus(it);
			return;
		}
	}
...
recti GUIFormSpecMenu::getAbsoluteRect()
{
	recti rect = AbsoluteRect;
	rect.ULC.Y += m_tabheader_upper_edge;
	return rect;
}

v2i GUIFormSpecMenu::getElementBasePos(const std::vector<std::string> *v_pos)
{
	v2f pos_f = v2f(padding.X, padding.Y) + pos_offset * spacing;
	if (v_pos) {
		pos_f.X += stof((*v_pos)[0]) * spacing.X;
		pos_f.Y += stof((*v_pos)[1]) * spacing.Y;
	}
	return v2i(pos_f.X, pos_f.Y);
}

v2i GUIFormSpecMenu::getRealCoordinateBasePos(const std::vector<std::string> &v_pos)
{
	return v2i((stof(v_pos[0]) + pos_offset.X) * imgsize.X,
		(stof(v_pos[1]) + pos_offset.Y) * imgsize.Y);
}

v2i GUIFormSpecMenu::getRealCoordinateGeometry(const std::vector<std::string> &v_geom)
{
	return v2i(stof(v_geom[0]) * imgsize.X, stof(v_geom[1]) * imgsize.Y);
}
...
void GUIFormSpecMenu::parseScrollContainer(parserData *data, const std::string &element)
{
	...
	if (parts.size() >= 6 && !parts[5].empty()) {
        ...
        bool use_y = orientation == "vertical" ? 1 : 0;

        if (use_y)
            content_padding_px = getRealCoordinateGeometry(v_size).Y;
        else
            content_padding_px = getRealCoordinateGeometry(v_size).X;
	}

	MY_CHECKPOS("scroll_container", 0);
	MY_CHECKGEOM("scroll_container", 1);

	v2i pos = getRealCoordinateBasePos(v_pos);
	v2i geom = getRealCoordinateGeometry(v_geom);
	...
	// make clipper
	recti rect_clipper = recti(pos, pos + geom);
	...
	recti rect_mover = recti(0, 0, geom.X, geom.Y);
...
void GUIFormSpecMenu::parseList(parserData *data, const std::string &element)
{
	...
	v2f slot_scale = style.getVector2f(StyleSpec::SIZE, v2f(0, 0));
	v2f slot_size(
	...
	v2f slot_spacing = style.getVector2f(StyleSpec::SPACING, v2f(-1, -1));
	v2f default_spacing = data->real_coordinates ?
			v2f(imgsize.X * 0.25f, imgsize.Y * 0.25f) :
			v2f(spacing.X - imgsize.X, spacing.Y - imgsize.Y);
	...
	v2i pos = data->real_coordinates ? getRealCoordinateBasePos(v_pos) :
			getElementBasePos(&v_pos);

	recti rect = recti(pos.X, pos.Y,
	...
	GUIInventoryList *e = new GUIInventoryList(Environment, data->current_parent,
			...
			v2i(slot_size.X, slot_size.Y), slot_spacing, this,
...
void GUIFormSpecMenu::parseCheckbox(parserData* data, const std::string &element)
{
    ...
    const v2u label_size = m_font->getTextSize(wlabel.c_str());
    s32 cb_size = Environment->getSkin()->getSize(EGDS_CHECK_BOX_WIDTH);
...
void GUIFormSpecMenu::parseScrollBar(parserData* data, const std::string &element)
{
	...
	recti rect =
			recti(pos.X, pos.Y, pos.X + dim.X, pos.Y + dim.Y);
...
void GUIFormSpecMenu::parseImage(parserData* data, const std::string &element)
{
    ...
    img::Image *texture = m_cache->getOrLoad<img::Image>(ResourceType::IMAGE, name);
...
void GUIFormSpecMenu::parseAnimatedImage(parserData *data, const std::string &element)
{
    ...
    gui::IGUIImage *e = Environment->addImage(rect, data->current_parent, spec.fid, nullptr, true);

    AtlasTileAnim anim_params = {frame_duration, frame_count};
    std::optional<u32> frame_offset;
...
void GUIFormSpecMenu::parseItemImage(parserData* data, const std::string &element)
{
	...
	GUIItemImage *e = new GUIItemImage(Environment, data->current_parent, spec.fid,
			recti(pos, pos + geom), name, m_font, m_client);
...
void GUIFormSpecMenu::parseButton(parserData* data, const std::string &element)
{
	...
	if (data->real_coordinates) {
		pos = getRealCoordinateBasePos(v_pos);
		geom = getRealCoordinateGeometry(v_geom);
		rect = recti(pos.X, pos.Y, pos.X+geom.X,
			pos.Y+geom.Y);
	} else {
		pos = getElementBasePos(&v_pos);
		...
		rect = recti(pos.X, pos.Y - m_btn_height,
    ...
    GUIButton *e = GUIButton::addButton(Environment, rect,
			data->current_parent, spec.fid, spec.flabel.c_str());
...
bool GUIFormSpecMenu::parseMiddleRect(const std::string &value, recti *parsed_rect)
{
	recti rect;
	...
	if (v_rect.size() == 1) {
		...
		rect.ULC = v2i(stoi(v_rect[0]), stoi(v_rect[1]));
		rect.LRC = v2i(stoi(v_rect[2]), stoi(v_rect[3]));
	} else {
...
void GUIFormSpecMenu::parseBackground(parserData* data, const std::string &element)
{
    ...
    GUIBackgroundImage *e = new GUIBackgroundImage(Environment, data->background_parent.get(),
            spec.fid, rect, name, middle, clip, autoclip_offset);
...
void GUIFormSpecMenu::parseTable(parserData* data, const std::string &element)
{
	...
	recti rect = recti(pos.X, pos.Y, pos.X+geom.X, pos.Y+geom.Y);
    ...
    GUITable *e = new GUITable(Environment, data->current_parent, spec.fid, rect);
...
void GUIFormSpecMenu::parseDropDown(parserData* data, const std::string &element)
{
	...
	if (data->real_coordinates) {
		...
		pos = getRealCoordinateBasePos(v_pos);
		geom = getRealCoordinateGeometry(v_geom);
		rect = recti(pos.X, pos.Y, pos.X+geom.X, pos.Y+geom.Y);
	} else {
		pos = getElementBasePos(&v_pos);

		s32 width = stof(parts[1]) * spacing.Y;

		rect = recti(pos.X, pos.Y,
...
void GUIFormSpecMenu::parsePwdField(parserData* data, const std::string &element)
{
    ...
    e->setOverrideColor(style.getColor(StyleSpec::TEXTCOLOR, img::white));
    e->setOverrideFont(style.getFont(m_fontmgr));
	...
	std::vector<std::string> &parts)
{
	...
	v2i pos = getElementBasePos(nullptr);
...
void GUIFormSpecMenu::parseHyperText(parserData *data, const std::string &element)
{
	...
	GUIHyperText *e = new GUIHyperText(spec.flabel.c_str(), Environment,
            data->current_parent, spec.fid, rect, m_client);
	e->drop();
...
void GUIFormSpecMenu::parseLabel(parserData* data, const std::string &element)
{
    ...
    render::TTFont *font = style.getFont(m_fontmgr);
	if (!font)
		font = m_font;
	...
	for (size_t i = 0; str_pos < str.size(); ++i) {
		...
		if (data->real_coordinates) {
			...
			rect = recti(
				pos.X, pos.Y,
                pos.X + font->getTextSize(line.c_str()).X,
				pos.Y + imgsize.Y);
        ...
        gui::IGUIStaticText *e = Environment->addStaticText(
                line.c_str(), rect, false, false, data->current_parent,
				spec.fid);
        e->setTextAlignment(EGUIA_UPPERLEFT, EGUIA_CENTER);
...
void GUIFormSpecMenu::parseVertLabel(parserData* data, const std::string &element)
{
    ...
    gui::IGUIStaticText *e = Environment->addStaticText(spec.flabel.c_str(),
...
void GUIFormSpecMenu::parseImageButton(parserData* data, const std::string &element)
{
	...
	recti rect = recti(pos.X, pos.Y, pos.X+geom.X,
		pos.Y+geom.Y);
    ...
    GUIButtonImage *e = GUIButtonImage::addButton(Environment, rect,
			data->current_parent, spec.fid, spec.flabel.c_str());
...
void GUIFormSpecMenu::parseTabHeader(parserData* data, const std::string &element)
{
	...
	if (data->real_coordinates) {
		...
		v2f pos_f = pos_offset * spacing;
		pos_f.X += stof(v_pos[0]) * spacing.X;
		pos_f.Y += stof(v_pos[1]) * spacing.Y - m_btn_height * 2;
		pos = v2i(pos_f.X, pos_f.Y);
	...
	for (const std::string &button : buttons) {
        ...
        tab->setTextColor(style.getColor(StyleSpec::TEXTCOLOR, img::white));
	}
	...
	m_fields.push_back(spec);
	m_tabheader_upper_edge = MYMIN(m_tabheader_upper_edge, rect.ULC.Y);
}
...
void GUIFormSpecMenu::parseItemImageButton(parserData* data, const std::string &element)
{
	...
	GUIButtonItemImage *e_btn = GUIButtonItemImage::addButton(Environment,
            rect, data->current_parent, spec_btn.fid, spec_btn.flabel.c_str(),
			item_name, m_client);
...
void GUIFormSpecMenu::parseBox(parserData* data, const std::string &element)
{
	...
	img::color8 tmp_color;
	std::array<img::color8, 4> colors;
    img::color8 null_color = img::colorU32NumberToObject(0x0);
    std::array<img::color8, 4> bordercolors = {null_color, null_color, null_color, null_color};
	...
	if (parseColorString(parts[2], tmp_color, true, 0x8C)) {
        ...
        colors = style.getColorArray(StyleSpec::COLORS, {null_color, null_color, null_color, null_color});
        bordercolors = style.getColorArray(StyleSpec::BORDERCOLORS,
            {null_color, null_color, null_color, null_color});
		borderwidths = style.getIntArray(StyleSpec::BORDERWIDTHS, {0, 0, 0, 0});
	}

	recti rect(pos, pos + geom);
...
void GUIFormSpecMenu::parseTooltip(parserData* data, const std::string &element)
{
	...
	// Read colors
	img::color8 bgcolor = m_default_tooltip_bgcolor;
	img::color8 color   = m_default_tooltip_color;
...
void GUIFormSpecMenu::parseModel(parserData *data, const std::string &element)
{
    ...
    GUIScene *e = new GUIScene(m_client, Environment,
...
void GUIFormSpecMenu::regenerateGui(v2u screensize)
{
	...
	mydata.size = v2i(100, 100);
	mydata.screensize = screensize;
	mydata.offset = v2f(0.5f, 0.5f);
	mydata.anchor = v2f(0.5f, 0.5f);
	mydata.padding = v2f(0.05f, 0.05f);
    ...
    m_default_tooltip_bgcolor = img::color8(img::PF_RGBA8,110,130,60,255);
    m_default_tooltip_color = img::white;

	// Add tooltip
	{
		assert(!m_tooltip_element);
		// Note: parent != this so that the tooltip isn't clipped by the menu rectangle
        m_tooltip_element = Environment->addStaticText(L"",
			recti(0, 0, 110, 18));
	...
	if (mydata.explicit_size) {
		...
		imgsize = v2i(use_imgsize, use_imgsize);
		spacing = v2f(use_imgsize*5.0/4, use_imgsize*15.0/13);
		padding = v2i(use_imgsize*3.0/8, use_imgsize*3.0/8);
		m_btn_height = use_imgsize*15.0/13 * 0.35;

        m_font = m_fontmgr->getDefaultFont();
	...
	sanity_check(skin);
	render::TTFont *old_font = skin->getFont();
	skin->setFont(m_font);
	...
	// clipping to `this`.)
	recti background_parent_rect(0, 0, 100000, 100000);
	...
	if (mydata.simple_field_count > 0 && !mydata.explicit_size) {
...

		{
            ...
            GUIButton::addButton(Environment, mydata.rect, this, 257,
	...
	if (!focused_element
            ...
            || focused_element->getType() == EGUIET_TAB_CONTROL)
		setInitialFocus();
...
void GUIFormSpecMenu::getAndroidUIInput()
{
	...
	for (const FieldSpec &field : m_fields) {
        ...
        if (dialog_type == porting::TEXT_INPUT && element_type == EGUIET_EDIT_BOX) {
...
void GUIFormSpecMenu::drawMenu()
{
	...
	*/
	...
	for (const auto &pair : m_tooltip_rects) {
		const recti &rect = pair.first->getAbsoluteClippingRect();
	...
	*/
	...
	if (!hovered_element_found) {
        ...
        if (current_cursor_icon != CI_NORMAL)
...
void GUIFormSpecMenu::showTooltip(const std::wstring &text,
    const img::color8 &color, const img::color8 &bgcolor)
{
    ...
    m_tooltip_element->setText(text.c_str());
    ...
    if (m_client->getInputHandler()->getReceiver()->getLastPointerType() == PointerType::Touch) {
		tooltip_offset_x *= 3;
		tooltip_offset_y  = 0;
		...
		for (const GUIFormSpecMenu::FieldSpec &s : m_fields) {
			if (s.send) {
				...
				if (s.ftype == f_Button) {
                    ...
                    if ((element) && (element->getType() == EGUIET_COMBO_BOX)) {
                    ...
                    if ((element) && (element->getType() == EGUIET_TAB_CONTROL)) {
					...
					if (element && element->getType() == EGUIET_ELEMENT)
...
bool GUIFormSpecMenu::preprocessEvent(const core::Event& event)
{
	...
	if (event.Type == EET_MOUSE_INPUT_EVENT &&
		...
		if (hovered && isMyChild(hovered) &&
                hovered->getType() == EGUIET_TAB_CONTROL) {
            GUISkin* skin = Environment->getSkin();
			sanity_check(skin != NULL);
			render::TTFont *old_font = skin->getFont();
			skin->setFont(m_font);
	...
	if (event.Type == EET_KEY_INPUT_EVENT) {
		...
		if (kp == EscapeKey
			...
			if (focused && isMyChild(focused) &&
                    (focused->getType() == EGUIET_LIST_BOX ||
					focused->getType() == EGUIET_CHECK_BOX) &&
                    (focused->getParent()->getType() != EGUIET_COMBO_BOX ||
					event.KeyInput.Key != core::KEY_RETURN)) {
				OnEvent(event);
				return true;
			}
    ...
    if (event.Type == EET_JOYSTICK_INPUT_EVENT) {
        if (event.Joystick.Joystick != m_joystick->getJoystickId())
			return false;

        bool handled = m_joystick->handleEvent(event.Joystick);
		if (handled) {
...
bool GUIFormSpecMenu::OnEvent(const core::Event& event)
{
	if (event.Type==EET_KEY_INPUT_EVENT) {
		...
		if (event.KeyInput.PressedDown && kp == getKeySetting("keymap_toggle_debug")) {
            if (!m_client || m_client->getEnv().getLocalPlayer()->checkPrivilege("debug"))
	...
	if ((event.Type == EET_MOUSE_INPUT_EVENT &&
			...
			event.Type == EET_TOUCH_INPUT_EVENT) {
		...
		if (event.Type == EET_MOUSE_INPUT_EVENT) {
			...
			case EMIE_MOUSE_WHEEL:
                button = (event.MouseInput.WheelDelta > 0) ?
		...
		if (event.Type == EET_TOUCH_INPUT_EVENT) {
            if (event.TouchInput.Type == TIE_LEFT_UP)
				touch = BET_RIGHT;
		}
